...
@app.post("/register", response_model=schemas.UserResponse, dependencies=[Depends(auth.verify_static_token)])
def register_user(user: schemas.UserCreate, db: Session = Depends(get_db)):
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=400, detail="Username atau email sudah digunakan") from e
...
# Endpoint untuk login - Mengembalikan JWT token
@app.post("/login", response_model=schemas.Token, dependencies=[Depends(auth.verify_static_token)])
